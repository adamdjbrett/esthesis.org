const BASE_URL = 'https://demo.axcora.com/wp';
// CUSTOM WP API
...
const USERS_API_URL = `${API_ROOT}/wp/v2/users`;
...
// ==========================================================
// FUNGSI ASYNC: MENGAMBIL PENGGUNA/PENULIS
// ==========================================================
async function fetchUsers() {
    try {
        console.log("Fetching users/authors...");
        // Fetch semua pengguna. Biasanya Anda hanya perlu pengguna dengan postingan.
        // Tambahkan '&roles=author,editor,administrator' jika perlu memfilter.
        const response = await fetch(USERS_API_URL); 

        if (!response.ok) {
            console.error(`Gagal mengambil pengguna: ${response.statusText}`);
            return [];
        }

        const users = await response.json();
        
        // Memproses dan menyederhanakan data pengguna
        const processedUsers = users.map(user => {
            return {
                ...
                // Mengambil URL avatar 96px, atau null jika tidak ada
                avatar_urls: user.avatar_urls?.['96'] || null, 
                // Anda bisa menambahkan jumlah postingan (count) jika diperlukan
                posts_count: user.count || 0 
            };
        });

        console.log(`✅ Users fetched: ${processedUsers.length}`);
...
console.log("Starting parallel fetch for Posts, Config, and Pages..."); 
    const [posts, config, pages, tags , users] = await Promise.all([
        fetchPosts(),
        fetchConfig(),
        fetchPages(),
        fetchTags(),
